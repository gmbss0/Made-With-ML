...
def tune_models(
    ...
    # Dataset
    ds = data.load_dataset(path=dataset_loc, n=train_loop_config.get("num_samples", None))
    ...
    tune_config = tune.TuneConfig(
        ...
        scheduler=scheduler,
        num_samples=num_runs,  # amount of hyperparameter configurations to sample
    )

    # Tuner
    tuner = Tuner(
        trainable=trainer,  # the training function that is executed by different workers
        run_config=run_config,  # logging and other callbacks
        param_space=param_space,  # hyperparameter space
        tune_config=tune_config,  # tuning configuration
    )
