...
from typing_extensions import Annotated  # moved to typing since Python 3.9
import tempfile
...
from madewithml.models import LanguageModel
...
def train_step(
    ...
    Args:
        ds (Dataset): Ray dataset to iterate batches from.
    ...
    for i, batch in enumerate(ds_generator):
        ...
        J = loss_fn(pred, targets)  # compute loss
...
def eval_step(
    ...
    with torch.inference_mode():
        for i, batch in enumerate(ds_generator):
            pred = model(batch)
            targets = F.one_hot(batch["targets"], num_classes=num_classes).float()  # one-hot (for loss_fn)
            J = loss_fn(pred, targets).item()
            loss += (J - loss) / (i + 1)
            y_trues.extend(batch["targets"].cpu().numpy())
            y_preds.extend(torch.argmax(pred, dim=1).cpu().numpy())
...
def train_loop_per_worker(config: dict) -> None:  # pragma: no cover, tested via train workload
    ...
    # Get datasets
    utils.set_seeds()
    train_ds = train.get_dataset_shard("train")  # get shard for worker
    val_ds = train.get_dataset_shard("val")

    # Model
    llm = BertModel.from_pretrained("allenai/scibert_scivocab_uncased", return_dict=False)
    model = LanguageModel(lmodel=llm, embedding_dim=llm.config.hidden_size, num_classes=num_classes)
    model = train.torch.prepare_model(model)  # this allows the same code to be executed independent of device type
...
def train_model(
    ...
    # Scaling config
    # -> this defines how to scale up the distributed training across workers
    ...
    checkpoint_config = CheckpointConfig(
        num_to_keep=1, # keep n best checkpoints
    ...
    # Run config
    # -> train.report will now log to MLFlow using the MLFlow callback
    # -> storage_path can be a local dir or cloud storage
    ...
    # Dataset
    ds = data.load_dataset(path=dataset_loc, n=train_loop_config["num_samples"])
    train_ds, val_ds = data.split_stratify(ds, col="tag", split=0.2)
    ...
    trainer = TorchTrainer(
        train_loop_per_worker=train_loop_per_worker,  # what should be done by each worker
        train_loop_config=train_loop_config,  # hyperparameters
        scaling_config=scaling_config,  # how to scale train_loop_per_worker
        run_config=run_config,  # general settings of run
        datasets={"train": train_ds, "val": val_ds},  # data
        dataset_config=dataset_config,  # data related config
